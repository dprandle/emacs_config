...
(global-set-key (kbd "M-q") 'pop-global-mark)
...
(define-key ctl-x-map (kbd "C-m") 'open-scratch-buffer)
(define-key ctl-x-map (kbd "C-n") 'open-shared-notes)
(define-key ctl-x-map (kbd "C-,") 'open-settings)
(define-key ctl-x-map (kbd "C-.") 'projectile-edit-dir-locals)
(define-key ctl-x-map (kbd "C-p") 'projectile-switch-project)
(define-key ctl-x-map (kbd "C-g") 'magit-status)

(define-key goto-map (kbd "s") 'magit-stage-modified)
(define-key goto-map (kbd "S") 'magit-unstage-all)
(define-key goto-map (kbd "c") 'magit-commit)
(define-key goto-map (kbd "p") 'magit-push)
(define-key goto-map (kbd "f") 'magit-pull)
(define-key goto-map (kbd "S-f") 'magit-fetch-all-prune)
(define-key goto-map (kbd "b") 'magit-branch)
...
(add-hook 'c-mode-hook 'add-func-calls-to-name-face)
(add-hook 'c++-mode-hook 'add-func-calls-to-name-face)
...
(custom-set-variables
 ...
 '(package-selected-packages
   '(modern-cpp-font-lock clang-format lsp-mode lsp-ui yasnippet lsp-treemacs helm-lsp projectile hydra flycheck company avy which-key helm-xref dap-mode multiple-cursors))
 '(tool-bar-mode nil))
(custom-set-faces
 ...
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(default ((t (:inherit nil :extend nil :stipple nil :background "#1e1e1e" :foreground "wheat1" :inverse-video nil :box nil :strike-through nil :overline nil :underline nil :slant normal :weight normal :height 142 :width normal :foundry "nil" :family "Courier New"))))
